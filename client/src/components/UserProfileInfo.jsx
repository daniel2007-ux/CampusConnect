...
const UserProfileInfo = ({user, posts, profile_Id, setShowEdit}) => {
  return (
      ...
      <div className='flex flex-col md:flex-row items-start gap-6'>
        ...
        <div className='w-full pt-16 md:pt-0 md:pl-36'>
            <div className='flex flex-col md:flex-row items-start justify-between'>
                ...
                {/* show edit button when it is that users profile */}
                {
                    !profile_Id && <button onClick={()=> setShowEdit(true)} className='flex items-center gap-2 border border-gray-300 hover:bg-gray-50 px-2 py-2 rounded-lg font-medium transition-colors mt-2 md:mt-0 cursor-pointer' > 
                        <PenBox w-4 h-4/> Edit
                    </button>
                }
            </div>
